"""政策顾问 API - 秦皇岛文旅项目政策顾问（基于政府政策知识库的智能问答 Agent）。
...
ADVISOR_SYSTEM_PROMPT = """你是「秦皇岛地区文旅项目政策顾问」——一个基于政府政策知识库的智能问答 Agent。

你的职责：基于下方提供的【政策库检索结果】和【联网搜索结果】，针对用户描述的文旅项目，给出政策红利、项目风险和可行性判断。联网搜索结果来自真实检索，source 字段填网站名、source_url 填真实 URL、source_type 填"网"。

铁律（违反即失败）：
1. 只用【政策库检索结果】和【联网搜索结果】里真实存在的政策。绝不能编造文号、编造来源、编造金额。
2. 每条红利的 source 必须填真实来源：政策库政策填发文单位（如"河北省住建厅"），联网结果填网站名。source_type 填 "库" 或 "网"。
...
_STOPWORDS = {
    ...
    "我在", "我想", "我要", "我有", "我做", "我想做", "我在秦皇岛",
}

# 文旅领域高频有效词，优先保留
_DOMAIN_HINTS = {
    "文旅", "文化", "旅游", "景区", "民宿", "酒店", "夜游", "夜经济", "夜间经济",
    "城市更新", "老旧小区", "历史风貌", "非遗", "文化遗产", "博物馆", "文化公园",
    "乡村振兴", "农旅", "田园", "康养", "体育", "赛事", "演艺", "文创", "IP",
    "补贴", "专项资金", "贷款", "税收", "减免", "奖补", "申报", "立项", "用地",
}
...
def _extract_keywords(text: str) -> list[str]:
    ...
    把「老旧小区」拆进「秦皇岛老旧小」导致 DOMAIN_HINTS 匹配不上。
    """
    text_str = text or ""
    seen: set[str] = set()
    ordered: list[str] = []
...
async def _search_policies(scenario: str, limit: int = 10) -> list[Policy]:
    ...
    region_cond = or_(
        PolicySource.region.like("%秦皇岛%"),
        PolicySource.region.like("%河北%"),
...
async def analyze_scenario(req: AdvisorAnalyzeRequest):
    ...
    if req.use_web:
        try:
            ...
            web_query = "秦皇岛 文旅 政策 " + " ".join(kws) if kws else "秦皇岛 文旅 政策 2026"
    ...
    # 3. 组装 user prompt
    user_prompt = f"""## 政策库检索结果（秦皇岛/河北/国家级，与项目相关）
{policy_context}
...
"""
    ...
    extra_body: Optional[dict] = None  # 不再用 plugins web_search（端点不支持）
